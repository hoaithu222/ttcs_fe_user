...
const InfoShop: React.FC = () => {
  ...
  useEffect(() => {
    if (shopInfo) {
      ...
      if (shopInfo.coverImage) {
        setCoverFile({ url: shopInfo.coverImage });
      }
    }
  }, [shopInfo]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    dispatch(
      updateShopStart({
        ...formData,
        logo: logoFile?.url || formData.logo,
        coverImage: coverFile?.url || formData.coverImage,
      } as any)
    );
  };
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-neutral-6">
              {canEdit
                ...
                : "Xem thông tin cửa hàng (chỉ xem)"}
            </p>
          </div>
          {!canEdit && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base font-semibold text-neutral-9">
                    {shopInfo.isActive ? "Đang hoạt động" : "Ngừng hoạt động"}
                  </p>
                </div>
                <div>
